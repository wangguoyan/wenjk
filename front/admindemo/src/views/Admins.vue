<template>
	<div>
		...
		<div style="padding: 10px 0;">
			...
			<el-upload :action="'http://' + 'localhost:8888/admins/import'" :show-file-list="false" accept="xlsx"
...
<script>
	...
	export default {
		...
		methods: {
			// 分页查询
			load() {
				axios.get("http://localhost:8888/admins/page?page=" + this.pageNum + "&limit=" + this.pageSize + "&username=" +
			...
			save() {
				...
				axios.post("http://localhost:8888/admins/save", this.form)
					.then(res => {
						// console.log(res)
						if (res.data.code == 0) {
			...
			// 删除
			del(id) {
				axios.get("http://localhost:8888/admins/del?id=" + id)
					.then(res => {
						if (res.data.code == 0) {
			...
			delbat() {
				...
				axios.post("http://localhost:8888/admins/delbat", ids)
					.then(res => {
						if (res.data.code == 0) {
							this.$message.success("删除成功！")
							this.load()
			...
			exp() {
				// alert(111)
				window.open(`http://localhost:8888/admins/export`)
			},
			// 导入成功后重新加载数据
